...
while i < len(lines):
    ...
    if line.find('class') != -1 or line.find('struct') != -1:
        ...
        # Method / function may already be implemented in header
        # Trailing means we are inside an implimentation
        if line.find('{') != -1 and line.find('namespace') == -1:
            trailing = True
        ...
        if line.find('(') != -1:
            ...
            if line.find('=') == -1:
                if inclass:
                    ...
                    space_found = method[0].rfind(' ', 0, start_paren - 1)
                    star_found = method[0].rfind('*', 0, start_paren - 1)
                    ampersand_found = method[0].rfind('*', 0, start_paren - 1)

                    start_method_name = max(space_found, max(star_found, ampersand_found))
